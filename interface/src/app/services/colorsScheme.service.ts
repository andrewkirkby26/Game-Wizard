...
export class ColorSchemeService {
    ...
    constructor(rendererFactory: RendererFactory2, public authenticationService: AuthenticationService) {
        // Create new renderer from renderFactory, to make it possible to use renderer2 in a service
        this.renderer = rendererFactory.createRenderer(null, null);

        let check = Utils.getLocalStorageVariable('theme');
        this.colorScheme = check ? check : 'light';
        this.update(this.colorScheme);
    }

    setDarkMode(on: boolean) {
        this.update(on ? 'dark' : 'light');
    }
    ...
    update(scheme: string) {
        ...
        Utils.setLocalStorageVariable('theme', scheme);
    }
